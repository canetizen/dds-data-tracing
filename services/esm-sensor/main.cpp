...
#define SERVICE_NAME "esm-sensor"
#define SENSOR_ID "ESM-2"
#define SENSOR_TYPE "ESM"
...
int main() {
    ...
    std::uniform_real_distribution<> alt_dis(2000.0, 12000.0);
    std::uniform_real_distribution<> hdg_dis(0.0, 360.0);
    std::uniform_real_distribution<> spd_dis(150.0, 800.0);
    std::uniform_real_distribution<> conf_dis(0.6, 0.85);
    std::uniform_int_distribution<> class_dis(0, 2);

    const char* classifications[] = {"UNKNOWN", "HOSTILE", "FRIEND"};

    int track_num = 1;

    printf("[%s] ESM sensor operational - publishing source tracks\n", SERVICE_NAME);

    while (running) {
        char track_id[32];
        snprintf(track_id, sizeof(track_id), "E-%d", track_num);
        ...
        if (writer.write(msg, "esm-detect")) {
            printf("[ESM] Track %s | Pos: %.2f, %.2f | Alt: %.0fm | Conf: %.2f\n",
